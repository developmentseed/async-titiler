...
from rio_tiler.io import AsyncMultiBaseReader
from rio_tiler.mosaic.backend import AsyncBaseBackend
...
from titiler.mosaic.factory import MosaicInfo, MosaicTilerFactory

from ..factories import AsyncTilerFactory, Info
from .backend import AsyncSTACAPIBackend
from .dependencies import (
    AssetsExprParams,
    AssetsParams,
    BackendParams,
    STACAPIExtensionParams,
    STACCollectionParams,
    STACItemParams,
)
from .readers import AsyncSTACReader
...
class AsyncMultiBaseTilerFactory(AsyncTilerFactory):
    ...
    router_prefix: str = "/collections/{collection_id}/items/{item_id}"
...
@define(kw_only=True)
class AsyncMosaicTilerFactory(MosaicTilerFactory):
    """Async MosaicTiler Factory."""

    backend: type[AsyncBaseBackend] = AsyncSTACAPIBackend
    backend_dependency: type[DefaultDependency] = BackendParams

    path_dependency: Callable[..., Any] = STACCollectionParams
    layer_dependency: type[DefaultDependency] = AssetsExprParams
    ...
    router_prefix: str = "/collections/{collection_id}"
        ...
        async def tile(
            ...
            async with self.backend(
                ...
                **backend_params.as_dict(),
                ...
                image, assets = await src_dst.tile(
                    x,
                    y,
                    z,
                    tilesize=tilesize,
            ...
            async with self.backend(
                ...
                **backend_params.as_dict(),
                ...
                for i, feature in enumerate(fc.features):
                    ...
                    image, assets = await src_dst.feature(
                        shape,
                        shape_crs=coord_crs or WGS84_CRS,
                        dst_crs=dst_crs,
                        align_bounds_with_dataset=True,
        ...
        async def bbox_image(
            ...
            async with self.backend(
                ...
                **backend_params.as_dict(),
                ...
                image, assets = await src_dst.part(
                    [minx, miny, maxx, maxy],
                    dst_crs=dst_crs,
        ...
        async def assets_for_bbox(
            ...
            async with self.backend(
                ...
                **backend_params.as_dict(),
                ...
                return await src_dst.assets_for_bbox(
                    minx,
                    miny,
                    maxx,
                    maxy,
        ...
        async def assets_for_lon_lat(
            ...
            async with self.backend(
                ...
                **backend_params.as_dict(),
                ...
                return await src_dst.assets_for_point(
                    lon,
                    lat,
                    coord_crs=coord_crs or WGS84_CRS,
                    **assets_accessor_params.as_dict(),
        ...
        async def assets_for_tile(
            ...
            async with self.backend(
                ...
                **backend_params.as_dict(),
                ...
                return await src_dst.assets_for_tile(
                    x,
                    y,
                    z,
                    **assets_accessor_params.as_dict(),
            ...
            async with self.backend(
                ...
                **backend_params.as_dict(),
                ...
                if ogc_params.bbox is not None:
                    ...
                    image, assets = await src_dst.preview(
                        search_options=assets_accessor_params.as_dict(),
                        pixel_selection=pixel_selection,
                        width=ogc_params.width,
                        height=ogc_params.height,
